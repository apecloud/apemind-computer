...
| 键 | 作用 | `0` | 出厂（建议） |
| --- | --- | --- | --- |
| `instance_memory_max_mb` | 每户 `memory.max` | 这一户不设内存顶 | 2048～4096 |
| `instance_pids_max` | 每户 `pids.max` | 这一户不设进程数顶 | 512～1024 |

`0` 跟现有 `idle_timeout_sec=0` 同一套习惯：运营明确关掉这一项。两个键相互独立，可以只放开内存、只放开进程数，或两个都放开。

写进 cgroup 文件的值必须是字面量 `max`，**不要写 `0`**。cgroup 里 `memory.max=0` 表示零字节，进程一进组就会被杀。settings 的 `0` 只是旋钮语义，落到内核是「无上限」。

即使两键都是 `0`，仍建 `dsh/<userKey>/` 这棵树（以后加 CPU、读 `memory.events` 还用得着），只是两个顶都写成 `max`。整容器 Helm limit 和 `agent/` 的 `memory.min` 不受影响：单户放开之后，这一户可以吃到容器还剩多少，仍可能把别人和网关挤死。
...
- `instance_memory_max_mb=0` 是不限制，还是把 cgroup 写成 0 字节？
