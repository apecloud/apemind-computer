...
没有 RSS 实测之前，默认宁可松（2–4Gi）。架构里写的活跃实例 300–500MB 是健康工作集，拿来当硬顶会误杀正常使用。落地时这两个键进 `/data/settings.json`，热更新规则跟现有旋钮一样，见 [host-settings.md](host-settings.md)。值是正整数或字面量 `"max"`。
...
| 键 | 合法值 | 作用 | 出厂（建议） |
| --- | --- | --- | --- |
| `instance_memory_max_mb` | 正整数，或 `"max"` | 每户 `memory.max`（整数按 MiB） | 2048～4096 |
| `instance_pids_max` | 正整数，或 `"max"` | 每户 `pids.max` | 512～1024 |

不限制就填 `"max"`，和 cgroup 文件同一个词，PUT / 文件里都是 JSON 字符串。两个键相互独立，可以只放开内存、只放开进程数，或两个都写成 `"max"`。

`0`、负数、其它字符串一律 400。cgroup 里 `memory.max=0` 是零字节，进程一进组就会被杀，旋钮不再借用 `0` 表示放开。`null` 仍回出厂，跟现有五键一样。

落到内核：正整数换算成字节或进程数再写；`"max"` 原样写入。两键都是 `"max"` 时仍建 `dsh/<userKey>/` 这棵树（以后加 CPU、读 `memory.events` 还用得着）。整容器 Helm limit 和 `agent/` 的 `memory.min` 不受影响：单户写成 `"max"` 之后，这一户可以吃到容器还剩多少，仍可能把别人和网关挤死。
...
要在真实宿主上数「同时能跑多少台」，用仓库里的 `scripts/density.py` 打控制面，不要手写一次性脚本。它只动 `density*` 键，默认退出就清掉。
